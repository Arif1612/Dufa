...
<div class="container">
    <div class="row justify-content-center py-5">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Transaction Successfull') }}</div>

                <div class="card-body">

                    {{ __('Payment Successfully Completed.') }}<br>
                    <form class="d-inline" method="POST" action="{{ route('verification.resend') }}">
                        @csrf
                        <button type="submit" class="btn btn-link p-0 m-0 align-baseline"><a href="/login">{{ __('Click here to go to dashboard') }}</a></button>
                    </form>
                </div>
            </div>
        </div>
